 ...
 * [[io.syspulse.ika.processor.impl.CacheProcessor.fromRpc3URI]] or
 * [[io.syspulse.ika.processor.rpc3.CacheRpc3Processor.fromRpc3URI]] to build processors).
 *
 * Formats:
 * - `none://` — no caching
...
final class Rpc3URI(uri: String) extends URILike {
  ...
  private def parse(u: String): Rpc3URI.Fields = {
    ...
    if (idx < 0) {
      return Rpc3URI.Fields("rpc3", DEF_TTL, DEF_TTL_LATEST, DEF_GC, ops)
    }
    ...
    scheme match {
      ...
      case "rpc3" =>
        ...
        if (ttl0.isDefined || latest0.isDefined || gc0.isDefined) {
          Rpc3URI.Fields("rpc3", ttl0.getOrElse(DEF_TTL), latest0.getOrElse(DEF_TTL_LATEST), gc0.getOrElse(DEF_GC), ops)
        } else if (body.isEmpty) {
          Rpc3URI.Fields("rpc3", DEF_TTL, DEF_TTL_LATEST, DEF_GC, ops)
        } else {
          commaParts(body) match {
            case ttl :: Nil =>
              Rpc3URI.Fields("rpc3", parseLong(ttl, DEF_TTL), DEF_TTL_LATEST, DEF_GC, ops)
            case ttl :: tl :: Nil =>
              Rpc3URI.Fields("rpc3", parseLong(ttl, DEF_TTL), parseLong(tl, DEF_TTL_LATEST), DEF_GC, ops)
            case ttl :: tl :: gc :: _ =>
              Rpc3URI.Fields("rpc3", parseLong(ttl, DEF_TTL), parseLong(tl, DEF_TTL_LATEST), parseLong(gc, DEF_GC), ops)
            case _ =>
              Rpc3URI.Fields("rpc3", DEF_TTL, DEF_TTL_LATEST, DEF_GC, ops)
          }
        }
      ...
      case _ =>
        Rpc3URI.Fields("expire", DEF_TTL, DEF_TTL_LATEST, DEF_GC, ops)
    }
  }
}
