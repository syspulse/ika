...
class RpcSessionLoadBalance(pool:Seq[String],rpcFailback:Long = 10000L, maxRetry:Int = 3,maxLaps:Int = 1) extends ProxySession(pool) {
...
class RpcPoolLoadBalance(pool:Seq[String])(implicit config:Config) extends ProxyPool {
  ...
  private val rpcFailback = 10000L
  private val rpcRetry = 3
  private val rpcLaps = 1

  // persistant pool
  val session = new RpcSessionLoadBalance(pool, rpcFailback, rpcRetry, rpcLaps)
}
