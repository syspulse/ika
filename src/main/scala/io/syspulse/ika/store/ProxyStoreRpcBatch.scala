...
class ProxyStoreRcpBatch(pool:ProxyPool)(implicit config:Config) extends ProxyStoreRcp(pool)(config) {

  import ProxyJson._

  // DEPRECATED: Cache removed, simplified batch processing
  override def batch(uri:String,req:String,headers:Seq[HttpHeader],session:ProxySession):Future[ProxyData] = {
