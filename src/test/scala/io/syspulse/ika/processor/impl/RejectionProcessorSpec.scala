...
class RejectionProcessorSpec extends AnyWordSpec with Matchers {
  ...
  "JsonRpcRejectionProcessor" should {
    ...
    "convert rejection to JSON-RPC 2.0 format" in {
      ...
      val session = Session(requestBody = ByteString("test")).copy(rejection = Some(rejection))

      val result = Await.result(processor.process(session), 5.seconds)

      result.responseBody shouldBe defined
      val body = result.responseBody.get.utf8String
    ...
    "pass through when no rejection" in {
      ...
      val session = Session(requestBody = ByteString("test"))
        .withResponse(ByteString("success"), ResponseSource.REMOTE)

      val result = Await.result(processor.process(session), 5.seconds)

      result.responseBody.map(_.utf8String) shouldBe Some("success")
  ...
  "RestApiRejectionProcessor" should {
    ...
    "map JSON-RPC codes to HTTP status codes" in {
      ...
      // Parse error -> 400
      val session1 = Session(requestBody = ByteString("test"))
      ...
      // Invalid request -> 400
      val session2 = Session(requestBody = ByteString("test"))
      ...
      val session3 = Session(requestBody = ByteString("test"))
      ...
      // Internal error -> 500
      val session4 = Session(requestBody = ByteString("test"))
  ...
  "CustomRejectionProcessor" should {
    ...
    "use custom formatter" in {
      val formatter = (r: Rejection, s: Session) => ByteString(s"""Custom error: ${r.message}""")
      ...
      result.responseBody.map(_.utf8String) shouldBe Some("Custom error: Test error")
    ...
    "access session data in formatter" in {
      val formatter = (r: Rejection, s: Session) => {
        ...
        ByteString(s"""{"error": "${r.message}", "requestId": "$reqId"}""")
      }
